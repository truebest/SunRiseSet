/*
 * Алгоритм расчета времени восхода и заката солнца в зависимости от координат и текущего времени
 * Применяется в составе прошивки на микроконтроллере
 * Для уменьшения размера используемых библиотек и памяти используется только float функции и переменные
 * Результатом работы алгоритма является расчет времени начала дня и ночи, конца дня и ночи
 * Адаптировал для себя: Шилин Павел 
 */
