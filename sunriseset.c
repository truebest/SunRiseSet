/*
 * јлгоритм расчета времени восхода и заката солнца в зависимости от координат и текущего времени
 * ѕримен€етс€ в составе прошивки на микроконтроллере
 * ƒл€ уменьшени€ размера используемых библиотек и пам€ти используетс€ только float функции и переменные
 * –езультатом работы алгоритма €вл€етс€ расчет времени начала дн€ и ночи, конца дн€ и ночи
 * јдаптировал дл€ себ€: Ўилин ѕавел 
 */
...
/**
 * \brief –ассчитывает номер дл€ в году по дате
 * \param day - день
 * \param month - мес€ц
 * \param year - год
 * \return - номер дн€
...
/**
 * \brief –асчет значений восхода и заката солнца. ќснован на алгоритме https://edwilliams.org/sunrise_sunset_algorithm.htm
 * \param is_sun_rise - восход или закат
 * \param day_num - номер дн€
 * \param lat - широта
 * \param lon - долгота
 * \param time_offset - смещение времени 
 * \param daylight_savings - лето\зима
 * \return результат - врем€ восхода\заката
 */
static float calcSunRiseSet(bool is_sun_rise, int day_num, float lat, float lon, int time_offset, int daylight_savings)
{
	...
	//1. ѕреобразуем долготу в часы из расчета что земл€ вращаетс€ со скоростью 15 градусов в час
	const float lng_hour = lon / 15;
	//2. ¬ зависимости от восхода или заката, что требуетс€ подсчитать
	float t = is_sun_rise ? day_num + ((6 - lng_hour) / DAY_HOURS) : day_num + ((18 - lng_hour) / DAY_HOURS);

	//3. —редн€€ аномали€ солнца, дл€ расчета удобно считать что солнце вращаетс€ вокруг земли (https://en.wikipedia.org/wiki/Position_of_the_Sun)
	const float m = (0.9856f * t) - 3.289f;

	//4. –ассчитываем истинную долготу —олнца
	const float l = fmodf(m + (1.916f * sinf(alpha * m)) + (0.020f * sinf(2 * alpha * m)) + 282.634, 360.0f);

	//5a. –ассчитываем пр€мое вхождение солнца (https://ru.wikipedia.org/wiki/ѕр€мое_восхождение)
	float ra = fmodf((180 / M_PI) * atanf(0.91764f * tanf(alpha * l)), 360.0f);

	//5b. «начение пр€мого восхождени€ должно быть в том же квадранте, что и L
	const float l_quadrant = floorf(l / 90) * 90;
	const float ra_quadrant = floorf(ra / 90) * 90;
	ra = ra + (l_quadrant - ra_quadrant);

	//5c. ѕравильное значение вхождени€ должно быть переведено в часы
	ra = ra / 15;

	//6. –ассчитываем склонение —олнца
	const float sin_dec = 0.39782f * sinf(alpha * l);
	const float cos_dec = cosf(asinf(sin_dec));

	//7a. –ассчитываем местный часовой угол —олнца
	const float cos_h = (cosf(alpha * ZENITH) - (sin_dec * sinf(alpha * lat))) / (cos_dec * cosf(alpha * lat));
	if (cos_h > 1) return ERROR_VAL;
	if (cos_h < -1) return -ERROR_VAL;
	

	//7b. «акончить расчет H и перевести в часы
	float h = is_sun_rise ? 360 - (180 / M_PI) * acosf(cos_h) : (180 / M_PI) * acosf(cos_h);
	h = h / 15;

	//8. –ассчитать местное среднее врем€ восхода / заката
	t = h + ra - (0.06571f * t) - 6.622f;

	//9. —корректировать обратно к UTC, преобразовать значение UT в местный часовой по€с широты / долготы
